...
fn main() raises:
    ...
    @parameter
    for fi in range(Variadic.size(ts.fn_types)):
...
struct TestSuite[*fn_types: Movable](Movable):
    ...
    fn add_tests(
        deinit self,
        var others: Tuple) -> TestSuite[
            *Variadic.concat[Self.fn_types, others.element_types]
        ]:
        return {self.tests.concat(others^), self.location}
    ...
    fn generate_report(
        ...
        for test_idx in range(Variadic.size(Self.fn_types)):
            var test_nm = get_type_name[Self.fn_types]()
            var error: Optional[Error] = None
            var start = perf_counter_ns()
            try:
                ref test = self.tests[test_idx]
                ref callable = trait_downcast[fn() raises unified](test)
                callable()
            except e:
                error = {e^}
